...
export const fetchRandomFilm = (): Promise<filmSchemaType> => {
  return axios
    .get(`${API_URL}/movie/random`)
    .then((res) => filmSchema.parse(res.data));
};
...
export const fetchFilmsGenre = (): Promise<genreListShemaType> => {
  return axios
    .get(`${API_URL}/movie/genres`)
    .then((res) => genreListSchema.parse(res.data));
};
...
export const addFavoriteFilm = (id: number): Promise<void> => {
  return axios
    .post(`${API_URL}/favorites`, new URLSearchParams({ id: id.toString() }), {
      headers: {
