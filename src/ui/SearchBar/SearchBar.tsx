...
export const SearchBar = () => {
  ...
  return (
    <div
      className={`${styles.header__search_wrapper} ${isSearchOpen ? styles.header__search_open : ""}`}
    >
      {isMobile ? (
        <>
          {isSearchOpen ? (
            <>
              ...
              <button
                ...
                className={styles.search_button_exit}
              >
                <svg
                  width="24"
                  height="24"
      ...
      {isLoading && (
        <ul className={styles.header__filtered_list}>
          <li className={styles.header__filtered_item}>Загрузка</li>
        </ul>
      )}
                  ...
                  <div className={styles.header__film_content}>
                    <div className={styles.header__film_top}>
                      <Rating
                        ...
                        imageURL={icon}
                      />
                      <span className={styles.header__film_year}>
                        {film.releaseYear}
                      </span>
                      <div className={styles.header__film_genre_wrapper}>
                        {film.genres.map((genre: string) => (
                          <span className={styles.header__genre} key={genre}>
                            {genre}
                          </span>
                        ))}
                      </div>
                      <span className={styles.header__film_time}>
                        {film.runtime} мин
                      </span>
                    </div>
                    <h3 className={styles.header__film_title}>
                      {film.originalTitle}
                    </h3>
                  </div>
                </div>
